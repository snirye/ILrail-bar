...
struct FavoriteRoute: Codable, Identifiable, Equatable {
    ...
    init(
        id: String = UUID().uuidString, name: String, fromStation: String, toStation: String,
        isDirectionReversed: Bool = false
    ) {
        self.id = id
        self.name = name
...
class PreferencesManager {
    ...
    var preferences: StationPreferences {
        get {
            guard let data = userDefaults.data(forKey: preferencesKey),
                let preferences = try? JSONDecoder().decode(StationPreferences.self, from: data)
            else {
    ...
    func savePreferences(
        fromStation: String, toStation: String, upcomingItemsCount: Int = 3,
        launchAtLogin: Bool = false,
        checkForUpdates: Bool = true,
        refreshInterval: Int = 300, activeDays: [Bool]? = nil, activeStartHour: Int = 6,
        activeEndHour: Int = 23,
    ...
    func saveFavoriteRoute(
        name: String, fromStation: String, toStation: String, isDirectionReversed: Bool
    ) {
        ...
        NetworkManager.shared.fetchStations { result in
            ...
            case .success(let remoteStations):
                ...
                if !stations.isEmpty {
                    ...
                    logDebug(
                        "Successfully loaded \(stations.count) stations in \(String(format: "%.2f", timeElapsed)) seconds"
                    )
...
extension Notification.Name {
    static let reloadPreferencesChanged = Notification.Name(
        "com.ilrailbar.reloadPreferencesChanged")
